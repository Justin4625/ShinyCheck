            ...
            if (key.startsWith("plza_shinyData_") || key.startsWith("sv_shinyData_")) {
                try {
                    ...
                    if (data && data.pokemonName && data.pokemonName.toLowerCase() === lowerName) {
                        ...
                        // Belangrijk: we hebben het originele ID uit de key nodig voor de delete functie
                        const originalId = keyParts[2];

                        items.push({
                            ...pokemon,
                            id: originalId, // Overschrijf met ID uit de opslag
                            storedData: data,
                            type: isPlza ? 'PLZA' : 'SV',
            ...
            <div onClick={(e) => e.stopPropagation()} className="bg-white border-2 border-slate-100 rounded-[2.5rem] shadow-2xl w-full max-w-xl max-h-[80vh] flex flex-col overflow-hidden">
                <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-white">
                    <div>
                        ...
                        <h2 className="text-2xl font-black uppercase italic text-slate-800">{pokemon.name} <span className="text-[#ff4d29]">x{capturedItems.length}</span></h2>
                    </div>
