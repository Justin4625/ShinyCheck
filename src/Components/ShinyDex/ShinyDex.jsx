...
export default function ShinyDex() {
    ...
    // --- POKÉMON GO THEME LOGICA (Actief tot 1 feb 2026) ---
    const isPogoTheme = useMemo(() => {
        const now = new Date();
        const expiryDate = new Date(2026, 1, 1, 0, 0, 0); // 1 Februari
    ...
    return (
        <div className={`relative min-h-screen p-4 sm:p-8 font-sans overflow-hidden transition-colors duration-1000 ${
            ...
            <div className={`absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full blur-[120px] pointer-events-none transition-colors duration-1000 ${
                isPogoTheme ? "bg-emerald-200/40" : "bg-orange-200/40"
            }`}></div>
            <div className={`absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full blur-[120px] pointer-events-none transition-colors duration-1000 ${
                isPogoTheme ? "bg-cyan-200/40" : "bg-cyan-200/40"
            }`}></div>

            <div className="relative z-10 max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 px-4 gap-6">
                    <div className="flex flex-col w-full md:w-auto">
                        <h1 className="text-4xl sm:text-6xl font-black tracking-tighter uppercase italic flex items-center text-slate-900">
                            Shiny<span className={isPogoTheme ? "text-emerald-500 drop-shadow-[0_0_10px_rgba(16,185,129,0.3)]" : "text-[#ff4d29]"}>Check</span>
                        ...
                        <div className="mt-4 w-full md:w-96">
                            ...
                            <div className={`h-3.5 w-full rounded-full overflow-hidden shadow-inner border transition-colors duration-1000 ${
                                isPogoTheme ? "bg-emerald-100 border-emerald-200" : "bg-slate-200 border-transparent"
                            }`}>
                                <div
                                    className={`h-full transition-all duration-1000 ${
                                        isPogoTheme ? "bg-gradient-to-r from-emerald-500 to-cyan-400 shadow-[0_0_15px_rgba(16,185,129,0.4)]" : "bg-[#ff4d29]"
                                    }`}
                    ...
                    <div className={`w-full md:w-80 backdrop-blur-xl border p-2 px-4 rounded-2xl flex items-center gap-3 transition-all duration-1000 ${
                        isPogoTheme ? "bg-white/80 border-emerald-100 shadow-lg" : "bg-white border-slate-200 shadow-sm"
                    }`}>
                        <input
                            type="text"
                            value={searchQuery}
                ...
                <div className={`mt-8 backdrop-blur-3xl rounded-[3.5rem] border p-6 sm:p-10 relative overflow-hidden transition-all duration-1000 ${
                    isPogoTheme ? "bg-white/60 border-emerald-100 shadow-2xl" : "bg-white/40 border-white shadow-xl"
                }`}>
                    <div className={`relative z-10 mb-10 flex flex-col lg:flex-row justify-between items-center gap-6 p-6 rounded-3xl border transition-all duration-1000 ${
                        isPogoTheme ? "bg-emerald-50/50 border-emerald-100 shadow-sm" : "bg-white/50 border-white shadow-sm"
                    }`}>
                        ...
                        <div className="flex flex-col items-center w-full max-w-md">
                            <div className="flex items-baseline gap-2 mb-2">
                                ...
                                <div className={`ml-3 px-3 py-1 rounded-lg border transition-all ${
                                    isPogoTheme ? "bg-emerald-100 border-emerald-200" : "bg-transparent border-transparent"
                                }`}>
                                    <span className={`font-black italic ${isPogoTheme ? "text-emerald-600" : "text-cyan-600"}`}>
                                        {regionStats.percentage}%
                                    </span>
                                </div>
                            </div>
                            <div className={`h-2.5 w-full rounded-full overflow-hidden border transition-colors ${
                                isPogoTheme ? "bg-emerald-50 border-emerald-100 shadow-inner" : "bg-slate-200 border-white"
                            }`}>
                                <div
                                    className={`h-full transition-all duration-1000 ${
                                        isPogoTheme ? "bg-gradient-to-r from-emerald-400 to-cyan-400" : "bg-gradient-to-r from-cyan-500 to-blue-500"
                                    }`}
                        ...
                        <div className={`flex items-center gap-4 p-3 px-5 rounded-2xl border transition-all ${
                            isPogoTheme ? "bg-emerald-50/50 border-emerald-100" : "bg-slate-100/50 border-slate-200"
                        }`}>
                            ...
                            <button
                                ...
                                className={`relative w-12 h-6 rounded-full transition-all duration-500 focus:outline-none ${
                                    showUnregisteredOnly
                                        ? (isPogoTheme ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.4)]' : 'bg-[#ff4d29]')
                                        : 'bg-slate-300'
                                }`}
                            >
