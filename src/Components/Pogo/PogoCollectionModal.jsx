...
export default function PogoCollectionModal({ data, onClose, pokemon, shinyIndex, originalId }) {
    ...
    return (
        /* onClick={onClose} verwijderd zodat klikken op achtergrond niets doet */
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mt-4">
                        {isEditing ? (
                            <>
                                ...
                                <button onClick={() => setIsEditing(false)} className="flex-1 py-3 bg-slate-200 text-slate-500 font-black italic rounded-2xl text-[11px] uppercase tracking-widest hover:bg-slate-300 active:scale-95 transition-all">Cancel</button>
                            </>
                        ) : (
                            <>
                ...
                {showConfirm && (
                    <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-30 flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-200">
                        ...
                        <div className="flex flex-col w-full gap-3">
                            <button onClick={deleteShinyLogic} className="w-full py-4 bg-red-500 text-white font-black uppercase italic text-xs tracking-widest rounded-2xl shadow-lg shadow-red-100 active:scale-95 transition-all">Yes, Delete Entry</button>
                            <button onClick={() => setShowConfirm(false)} className="w-full py-3 text-slate-400 font-black uppercase italic text-[10px] tracking-widest hover:text-slate-600 transition-colors">No, Keep it</button>
                        </div>
                    </div>
                )}
            </div>
