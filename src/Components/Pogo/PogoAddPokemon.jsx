...
export default function PogoAddPokemon({ pokemon, onClose, onSave }) {
    // We houden de state voor counter en timer op 0 omdat deze niet meer handmatig worden ingevoerd
    const counter = 0;
    const totalSeconds = 0;

    // Datum instellen op huidige tijd voor de input (gelokaliseerd)
    ...
    const handleSave = () => {
        const prefix = "pogo"; // Specifieke prefix voor Pokémon GO

        // Bepaal de volgende index voor deze pokemon in de pogo lijst
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[110] p-4" onClick={onClose}>
            <div
            ...
            >
                ...
                <div className="w-full space-y-6 relative z-10">
                    ...
                    <div className="flex flex-col gap-3 mt-4">
                        <button
                            onClick={handleSave}
                            className="w-full py-4 bg-emerald-500 text-white font-black rounded-2xl uppercase italic text-sm tracking-[0.2em] hover:bg-emerald-600 active:scale-95 transition-all shadow-lg shadow-emerald-200"
                        >
                            Confirm
                        </button>
                        <button
                            onClick={onClose}
                            className="w-full py-3 bg-slate-50 text-slate-400 font-black rounded-2xl uppercase italic text-[10px] tracking-widest hover:bg-slate-100 transition-colors"
                        >
                            Cancel
                        </button>
                    </div>
