...
export default function SvCollectionModal({ data, onClose, pokemon, shinyIndex, gameName, originalId }) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[60] p-4 overflow-hidden">
            <div onClick={(e) => e.stopPropagation()} className="relative bg-white border-2 border-orange-100 rounded-[2.5rem] shadow-2xl p-8 w-full max-w-xl flex flex-col items-center overflow-hidden transition-all">
                ...
                <button onClick={onClose} className="absolute top-5 right-5 w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:text-orange-500 transition-all z-20 group">
                    <span className="text-xl font-black">✕</span>
                </button>

                <div className="flex flex-col items-center mb-6 relative z-10 text-center">
                    <div className="px-3 py-1 bg-orange-500 rounded-full mb-3 shadow-lg shadow-orange-100">
                    ...
                    {isChangingPokemon ? (
                        <div className="w-full bg-slate-50 border border-slate-200 rounded-3xl p-4 shadow-inner flex flex-col gap-3 animate-in fade-in slide-in-from-top-4 duration-300">
                            <input
                                ...
                                className="w-full p-2.5 rounded-xl border-2 border-orange-100 font-bold text-xs outline-none focus:border-orange-500 transition-all"
                                value={searchQuery}
                        ...
                        <div className="flex flex-col items-center gap-4">
                            ...
                            {isEditing && (
                                <button
                                    ...
                                    className="bg-orange-100 text-orange-600 text-[9px] font-black px-4 py-2 rounded-xl uppercase tracking-widest hover:bg-orange-200 transition-all border border-orange-200"
                                >
                                    Change Species
                                </button>
                            )}
                ...
                <div className="w-full bg-slate-50/50 rounded-3xl border border-slate-100 p-4 flex flex-col gap-3 z-10 shadow-inner">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mt-3">
                        {isEditing ? (
                            <>
                                <button onClick={handleSave} className="flex-1 py-2.5 bg-orange-500 text-white font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-orange-600 shadow-lg active:scale-95 transition-all">Save Changes</button>
                            ...
                            <>
                                <button onClick={() => setIsEditing(true)} className="flex-1 py-2.5 bg-white border-2 border-orange-500 text-orange-600 font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-orange-50 active:scale-95 transition-all">Edit Entry</button>
