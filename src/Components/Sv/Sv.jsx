...
export default function Sv() {
    ...
    // EXACT DEZELFDE LOGICA ALS PLZA: Combineer alle data direct
    const allSvPokemon = svPokemon.concat(svTmPokemon, svIdPokemon);

    // Gebruik de volledige lijst voor de API fetch
    ...
    // Bepaal welke lijst getoond wordt, maar behoud de IDs
    const displayedPokemon =
        activeTab === "base" ? svPokemon :
            activeTab === "teal" ? svTmPokemon :
                activeTab === "indigo" ? svIdPokemon :
                    activeTab === "active" ? allSvPokemon : [];

    const filteredPokemon = displayedPokemon.map((p, index) => ({
        ...p,
    ...
    return (
        <div className="relative p-3 sm:p-6 min-h-screen bg-[#f8f9fa] overflow-hidden font-sans">
                    ...
                    <div className="flex justify-between items-end mb-1.5 px-1">
                        ...
                        <div className="flex items-baseline gap-2">
                            ...
                            <span className="text-xl font-black text-[#333] italic">
                                {shinyProgress.count}<span className="text-gray-300 mx-0.5 text-lg">/</span>662
                            </span>
                        </div>
                    </div>
                    <div className="relative h-2.5 w-full bg-gray-100 rounded-full overflow-hidden border border-gray-50">
                        <div className="absolute top-0 left-0 h-full bg-gradient-to-r from-[#ff4d00] via-[#ffcc00] to-[#8c00ff]" style={{ width: `${shinyPercentage}%` }}></div>
                    </div>
                </div>
            </div>
            ...
            <div className="relative z-10 max-w-6xl mx-auto mb-8 flex flex-col lg:flex-row items-center gap-4">
                ...
                <div className="w-full lg:w-[300px] bg-white p-3 rounded-lg shadow-sm border-b-2 border-gray-200 flex flex-col gap-2">
                    <div className="flex items-center justify-between px-1">
                        ...
                        <button onClick={() => setShowMissingOnly(!showMissingOnly)} className={`relative h-5 w-10 rounded-full transition-all duration-300 border ${showMissingOnly ? 'bg-[#ff4d00] border-[#ff4d00]' : 'bg-gray-100 border-gray-200'}`}>
                    ...
                    <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="SEARCH POKÉMON OR TYPE..." className="w-full pl-3 pr-8 py-2 bg-gray-50 border border-gray-200 rounded text-xs font-bold focus:border-[#ff4d00] outline-none transition-colors placeholder:text-gray-300 tracking-tight" />
                </div>
            </div>

            <div className="relative z-10 max-w-7xl mx-auto min-h-[400px]">
                {activeTab === "active" ? (
                    <SvActiveHunts svPokemon={allSvPokemon} pokemonList={pokemonList} formatTime={formatTime} openModal={openModal} />
                ) : (
            ...
            <SvModal
                ...
                onClose={closeModal}
                index={selectedPokemon ? allSvPokemon.findIndex(p => p.id === selectedPokemon.id) : -1}
            />
        </div>
    );
}