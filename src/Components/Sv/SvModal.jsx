...
export default function SvModal({ selectedPokemon, onClose, index = 0 }) {
    ...
    useEffect(() => {
        ...
        // Reset state voor elke nieuwe Pokémon klik
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div onClick={(e) => e.stopPropagation()} className="relative bg-white border-b-4 border-r-4 border-gray-300 rounded-tr-[30px] rounded-bl-[30px] rounded-tl-lg rounded-br-lg w-full max-w-2xl flex flex-col items-center overflow-hidden shadow-2xl">
                ...
                <div className="w-full pt-8 px-8 flex flex-col items-center">
                    ...
                    <h2 className="text-3xl font-black uppercase italic text-[#333] tracking-tighter">
                        {selectedPokemon.name}
                    </h2>
                </div>
                <div className="flex justify-center my-6 gap-2 z-10">
                    {["hunt", "settings"].map((tab) => (
                        <button key={tab} onClick={() => setActiveTab(tab)} className={`px-10 py-2.5 text-xs font-black italic tracking-widest transform -skew-x-12 border-b-2 ${activeTab === tab ? 'text-white border-black/10' : 'text-gray-400 border-gray-100'}`} style={{ backgroundColor: activeTab === tab ? accentColor : 'transparent' }}>
                            <span className="inline-block skew-x-[12deg]">{tab.toUpperCase()}</span>
                        </button>
                    ))}
                </div>
