...
function GamesModal({ isOpen, onClose, gameItems, currentPath }) {
    ...
    return (
        <div
            className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[110] p-4 md:p-8 animate-in fade-in duration-300"
            onClick={onClose}
        >
            <div
                className="relative bg-white border border-slate-200 rounded-[2rem] md:rounded-[2.5rem] shadow-[0_40px_100px_rgba(0,0,0,0.4)] w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Decoratie */}
                <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-cyan-100 via-cyan-500 to-cyan-100 opacity-30 shrink-0" />
                ...
                <div className="overflow-y-auto p-6 md:p-12 custom-scrollbar">
                    <div className="relative z-10 flex flex-col items-center">
                            ...
                            {gameItems.map((game) => {
                                ...
                                return (
                                    <Link
                                        ...
                                        onClick={onClose}
                                        className={`
                                            group relative p-1 transition-all duration-300 rounded-[1.5rem] md:rounded-[2rem]
                                            ${isActive ? "bg-cyan-500 shadow-lg scale-[1.02] md:scale-105" : "bg-slate-100 hover:bg-slate-200/50"}
                                        `}
                                    >
                                        <div className="bg-white rounded-[1.3rem] md:rounded-[1.8rem] p-4 md:p-8 h-full flex flex-row sm:flex-col items-center justify-start sm:justify-center text-left sm:text-center aspect-auto sm:aspect-[4/5] overflow-hidden relative border border-slate-100">
                                            {/* Logo Container - Kleiner op mobiel, flex-row voor horizontale layout op kleine schermen */}
                                            <div className="relative w-20 h-20 sm:w-full sm:h-32 flex items-center justify-center mb-0 sm:mb-8 shrink-0">
                                                <img
                                                    src={game.logo}
                                                    alt={game.label}
                                            ...
                                            <div className="flex flex-col items-start sm:items-center gap-2 ml-4 sm:ml-0">
                                                <span className={`text-[11px] md:text-[12px] font-black uppercase tracking-[0.1em] md:tracking-[0.15em] transition-colors ${isActive ? "text-slate-900" : "text-slate-400 group-hover:text-slate-900"}`}>
                                                    {game.label}
                                                </span>
                                                {isActive && (
                                                    <div className="px-3 py-1 bg-cyan-500 text-white text-[8px] md:text-[9px] font-black uppercase rounded-full flex items-center gap-2">
                        ...
                        <button
                            onClick={onClose}
                            className="mt-8 md:mt-12 group flex items-center gap-3 px-8 py-3.5 bg-slate-900 text-white hover:bg-cyan-600 transition-all rounded-xl text-[10px] md:text-[11px] font-black uppercase tracking-[0.3em] shadow-lg active:scale-95"
                        >
                            <span>✕</span>
                            Return
                        </button>
