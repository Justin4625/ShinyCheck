        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[70] p-4">
            ...
            >
                ...
                <div className="flex justify-center gap-4">
                    ...
                    <button
                        onClick={onConfirm}
                        className="flex-1 px-6 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white font-black italic rounded-xl text-[10px] uppercase tracking-widest shadow-lg shadow-amber-200 transition-all hover:scale-105 active:scale-95"
                    >
                        Delete
                    </button>
                </div>
...
export default function PlaCollectionModal({ data, onClose, pokemon, shinyIndex, gameName, originalId }) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[60] p-4">
            <div onClick={(e) => e.stopPropagation()} className="relative bg-white border-2 border-amber-100 rounded-[2.5rem] shadow-2xl p-8 w-full max-w-xl flex flex-col items-center overflow-hidden">
                ...
                <button onClick={onClose} className="absolute top-5 right-5 w-10 h-10 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:text-amber-500 transition-all z-20 group">
                    <span className="text-xl font-black">✕</span>
                </button>

                <div className="flex flex-col items-center mb-6 relative z-10 text-center">
                    <div className="px-3 py-1 bg-amber-500 rounded-full mb-3 shadow-lg shadow-amber-100">
                ...
                <div className="w-full bg-slate-50/50 rounded-3xl border border-slate-100 p-4 flex flex-col gap-3 z-10 shadow-inner">
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="flex flex-col items-center bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            {isEditing ? (
                                <input type="number" value={editCounter} onChange={(e) => setEditCounter(e.target.value)} className="w-full text-center text-xl font-black bg-amber-50 rounded-lg border-none focus:ring-2 focus:ring-amber-500" />
                            ) : (
                        ...
                        <div className="flex flex-col items-center bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            {isEditing ? (
                                <div className="flex gap-1 items-center">
                                    <input type="number" value={editHrs} onChange={(e) => setEditHrs(e.target.value)} className="w-9 sm:w-10 text-center font-black bg-amber-50 rounded text-sm" />
                                    <span className="text-[8px] font-bold text-amber-600">h</span>
                                    <input type="number" value={editMins} onChange={(e) => setEditMins(e.target.value)} className="w-9 sm:w-10 text-center font-black bg-amber-50 rounded text-sm" />
                                    <span className="text-[8px] font-bold text-amber-600">m</span>
                                    <input type="number" value={editSecs} onChange={(e) => setEditSecs(e.target.value)} className="w-9 sm:w-10 text-center font-black bg-amber-50 rounded text-sm" />
                                    <span className="text-[8px] font-bold text-amber-600">s</span>
                                </div>
                            ) : (
                        ...
                        <div className="flex flex-col items-center bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm col-span-2">
                            ...
                            {isEditing ? (
                                <input type="datetime-local" value={editTimestamp} onChange={(e) => setEditTimestamp(e.target.value)} className="w-full text-center text-sm font-black bg-amber-50 rounded-lg border-none focus:ring-2 focus:ring-amber-500" />
                            ) : (
                        ...
                        <div className="flex flex-col items-center bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm col-span-2">
                            ...
                            <p className="text-sm font-black italic text-amber-600 uppercase tracking-tighter">{gameName || "Pokémon Legends: Arceus"}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mt-3">
                        {isEditing ? (
                            <>
                                <button onClick={handleSave} className="flex-1 py-2.5 bg-amber-500 text-white font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-amber-600 shadow-lg active:scale-95 transition-all">Save Changes</button>
                            ...
                            <>
                                <button onClick={() => setIsEditing(true)} className="flex-1 py-2.5 bg-white border-2 border-amber-500 text-amber-500 font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-amber-50 active:scale-95 transition-all">Edit Entry</button>
