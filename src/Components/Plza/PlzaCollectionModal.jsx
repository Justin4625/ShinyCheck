    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[60] p-4">
            <div
                ...
                className="relative bg-white border-2 border-cyan-100 rounded-[2.5rem] shadow-2xl p-8 w-full max-w-xl flex flex-col items-center overflow-hidden"
            >
                ...
                <div className="w-full bg-slate-50/50 rounded-3xl border border-slate-100 p-4 flex flex-col gap-3 z-10 shadow-inner">
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="flex flex-col items-center bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            {isEditing ? (
                                <div className="flex gap-1 items-center">
                                    <input type="number" value={editHrs} onChange={(e) => setEditHrs(e.target.value)} className="w-9 sm:w-10 text-center font-black bg-cyan-50 rounded text-sm" placeholder="H"/>
                                    <span className="text-[8px] font-bold text-cyan-600">h</span>
                                    <input type="number" value={editMins} onChange={(e) => setEditMins(e.target.value)} className="w-9 sm:w-10 text-center font-black bg-cyan-50 rounded text-sm" placeholder="M"/>
                                    <span className="text-[8px] font-bold text-cyan-600">m</span>
                                    <input type="number" value={editSecs} onChange={(e) => setEditSecs(e.target.value)} className="w-9 sm:w-10 text-center font-black bg-cyan-50 rounded text-sm" placeholder="S"/>
                        ...
                        {/* OMDRAAI: Captured On staat nu boven Game */}
                        {!isEditing && (
                        ...
                        <div className="flex flex-col items-center bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm col-span-2">
                            ...
                            {isEditing ? (
                                <select
                                    ...
                                    className="w-full text-center font-black italic text-cyan-600 bg-cyan-50 rounded-lg border-none p-1 text-sm"
                                >
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 mt-3">
                        {isEditing ? (
                            <>
                                <button
                                    onClick={handleSave}
                                    className="flex-1 py-2.5 bg-cyan-500 text-white font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-cyan-600 shadow-lg shadow-cyan-100 active:scale-95 transition-all"
                                >
                                    Save Changes
                                </button>
                                <button
                                    onClick={() => setIsEditing(false)}
                                    className="flex-1 py-2.5 bg-slate-200 text-slate-500 font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-slate-300 active:scale-95 transition-all"
                                >
                                    Cancel
                                </button>
                            </>
                            ...
                            <>
                                <button
                                    ...
                                    className="flex-1 py-2.5 bg-white border-2 border-cyan-500 text-cyan-500 font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-cyan-50 shadow-sm active:scale-95 transition-all"
                                >
                                    Edit Entry
                                </button>
                                <button
                                    onClick={() => setShowConfirm(true)}
                                    className="px-8 py-2.5 bg-white border-2 border-slate-200 text-slate-400 font-black italic rounded-2xl text-[10px] uppercase tracking-[0.15em] hover:text-red-500 hover:border-red-200 transition-all shadow-sm active:scale-95"
                                >
                                    Delete
                                </button>
                            </>
