                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="flex flex-col items-center bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            {isEditing ? (
                                ...
                                <p className="text-2xl font-black italic text-slate-900">{data.counter}</p>
                            )}
                        </div>
                        {/* Duration */}
                        <div className="flex flex-col items-center bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            {isEditing ? (
                                ...
                                <p className="text-lg font-black italic text-slate-900">{formatTime(data.timer)}</p>
                            )}
                        </div>

                        {/* Captured On (Edit Mode toegevoegd) */}
                        ...
                        <div className="flex flex-col items-center bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm col-span-2">
                            ...
                            <p className="text-sm font-black italic text-cyan-600 uppercase">{gameName || "Pokémon Legends: Z-A"}</p>
                        </div>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-3 mt-3">
                        {isEditing ? (
                            <><button onClick={handleSave} className="flex-1 py-2.5 bg-cyan-500 text-white font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-cyan-600">Save</button>
                                <button onClick={() => setIsEditing(false)} className="flex-1 py-2.5 bg-slate-200 text-slate-500 font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-slate-300">Cancel</button></>
                        ) : (
                            <><button onClick={() => setIsEditing(true)} className="flex-1 py-2.5 bg-white border-2 border-cyan-500 text-cyan-500 font-black italic rounded-2xl text-[10px] uppercase tracking-widest hover:bg-cyan-50">Edit</button>
                                <button onClick={() => setShowConfirm(true)} className="px-8 py-2.5 bg-white border-2 border-slate-200 text-slate-400 font-black italic rounded-2xl text-[10px] uppercase hover:text-red-500">Delete</button></>
                        )}
                    </div>
                </div>
